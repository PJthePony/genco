...
const emit = defineEmits(['approve', 'skip', 'open-email'])

const showingAlts = ref(false)

function showAlts() { showingAlts.value = true }
function hideAlts() { showingAlts.value = false }
...
<template>
  <div class="card" :class="{ cleared: card.cleared }">
    ...
    <!-- Alt actions -->
    <div class="alt-actions" :class="{ visible: showingAlts }">
      <button v-if="card.hasBriefing" class="btn-alt" @click="approve(card.sender + ' added to Daily Briefing')">
      ...
      <button v-if="card.action !== 'Reply'" class="btn-alt" @click="approve('Reply draft created')">
      ...
      <button v-if="card.action !== '+ Task'" class="btn-alt" @click="approve('Task added to Tessio')">
      ...
      <button v-if="card.action !== 'Archive'" class="btn-alt" @click="approve('Archived')">
      ...
      <button v-if="card.action !== 'Unsubscribe'" class="btn-alt" @click="approve('Unsubscribed')">
